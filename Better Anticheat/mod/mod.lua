	...
	MenuCallbackHandler.pda_pause_callback = function(self)
		if Utils:IsInHeist() then
			if PDA.settings.pda_pause == false then
				...
				managers.chat:_receive_message(1, "Better Anticheat", "Pausado para este heist",tweak_data.system_chat_color)
				else
				PDA.settings.pda_pause = false
				managers.chat:_receive_message(1, "Better Anticheat", "Reactivado el anticheat",tweak_data.system_chat_color)			
			end
		end
	end
...
if RequiredScript == "lib/network/base/networkpeer" then
	...
	Hooks:PostHook(NetworkPeer, "mark_cheater", "Tagged Cheater", function(self)
		if PDA.settings.pda_pause == false and self ~= nil and self then
			log ("[BA] El juego detectÃ³ a "..(tostring(self:name())).." como cheater ")
...
function detect(tmate,rcode)
	...
	local character_id,peer_id
		...
		log ("[BA]  Detección comenzada para  "..(tostring(tmate._peer_id)))
			...
			log ("[BA]  La persona detectada es  "..(tostring(peer:name())))
		...
		if Network:is_server() == true and PDA.settings.kick == true then
			if peer then
				...
				if PDA.settings.punish == 5 then
					log ("[BA] Castigo de desagrado hasta muerte comenzado para el jugador: "..(tostring(peer:name())))
					bleed_out(peer,5)
				elseif PDA.settings.punish == 4 then
					log ("[BA] Castigo de patada de cloaker comenzado para el jugador: "..(tostring(peer:name())))
					incapacitated(peer,5)
				elseif PDA.settings.punish == 3 then
					log ("[BA] Castigo de elctrocuciÃ³n comenzado para el jugador: "..(tostring(peer:name())))
					tased(peer,5)
				elseif PDA.settings.punish == 2 then
					log ("[BA] Castigo de arresto comenzado para el jugador: "..(tostring(peer:name())))
					action(peer,5)
				end
		end
	end 
...
log ("[BA] Anuncio para el jugador: "..(tostring(peer:name())))
	if peer and PDA.settings.chat == 1 then
		local message1 = ("Better Anticheat : "..(tostring(peer:name()))..managers.localization:text("is_cuffed_for")..managers.localization:text(rcode)) 
	...
	elseif peer and PDA.settings.chat == 2 then
		managers.chat:_receive_message(1, "Better Anticheat", (tostring(peer:name()))..managers.localization:text("is_cuffed_for")..managers.localization:text(rcode),tweak_data.system_chat_color)
		log("message sent")
	end
end
...
function skilltest(peer)
	log ("[BA] Prueba de habilidades comenzada para :"..tostring(peer:name()))
	local number = 0
	local answer = 0
	local sum = 0
	...
	local answer = false
		if peer ~= nil and (peer:skills()) ~= nil and PDA.settings.pda_pause == false then
			...
			if #skills_perk_deck_info == 2 then
				...
				log ("[BA]  "..tostring(peer:name()).." tiene el ID de perk: "..tostring(perk_deck[1]))
				for i=1, #skills do
					number = tonumber(skills[i])
					sum = sum + number
				end
				log ("[BA]  "..tostring(peer:name()).." tiene los siguientes puntos de habilidad: "..tostring(sum))
				if peer and sum ~= nil and (peer:level()) ~= nil then
					if sum > 120 and PDA.settings.skilltest == true then
						answer = 1
						managers.chat:_receive_message(1, "Better Anticheat", (tostring(peer:name()))..managers.localization:text("has_count")..(tostring(sum)),tweak_data.system_chat_color)
						log ("[BA]  Puntos de habilidad infinitos encontrados en: "..(tostring(peer:name())))
					elseif peer and sum > (tonumber(peer:level()) + 2 * math.floor(tonumber(peer:level()) / 10)) then
						log ("[BA]  Puntos de habilidad mÃ¡s altos que los esperados de parte de: "..(tostring(peer:name())))
						answer = 1
					elseif (tonumber(perk_deck[1])) > 21 and PDA.settings.perktest == true or (tostring(perk_deck[1])) == nil then
						answer = 2
						log ("[BA] Perk-Deck de terceros detectada en: "..(tostring(peer:name())))
...
function p3dcheck(peer)
	if peer and PDA.settings.pda_pause == false then
			log ("[BA] P3DHack comprobado en el jugador:  "..(tostring(peer:name())))
		local name = tostring(peer:name())
		if string.find(name, "^%[P3DHack]") then
			log ("[BA] P3DHack encontrado en el jugador:  "..(tostring(peer:name())))
			punishment(peer,"p3d")
		else
			modtest(peer)
		end
...
log ("[BA] Prueba de MODs comenzada para: "..(tostring(peer:name())))

	if not peer:synced_mods() then
		managers.chat:_receive_message(1, "Better Anticheat", ("El jugador %j no tiene mods detectados, puede que sea un cheater, si es el caso deberá, manualmente expulsarlo o bloquearlo.").format('%s', peer:name()),tweak_data.system_chat_color)
	end
	...
	for i, mod in ipairs(peer:synced_mods()) do
		...
		log (" [BA] No se encotrÃ³ lista de MODs, nueva lista creada.")
		...
		local defaultlist = ("pirate perfection,p3dhack,dlc unlocker,skin unlocker,p3dunlocker,selective dlc unlocker,the great skin unlock,Keepers,Monkeepers,Removable Mask,Demask,Realtime XP,Infamy Pool Overflow,Print Faster,Berserk Helper,STOL3NH4CK,STOLENHACK,Gambler Cooldown reducer,No interaction cooldowns,Version A,No interaction cooldowns,Version B,Buy More Gage Asset,Infamy with active Crime Spree,Go Infamous with active Crime Spree,Jump and Duck in Casing Mode,[Big Bank] PC Helper,Big Bank Computer Hack BLT,Lock Smasher,More Infamy XP,Experience Booster Perk Deck,Ultimate Trainer,Silent Assassin,Berserker Live Matters,Meth Helper,Meth Helper (Updated),Better Bots,The Cooker,AutoCooker,Cook Faster,Carry Stacker Reloaded,CARRY STACKER: LIVE AND RELOADED,p3dhack,p3dhack free,P3DHack Free Version,p3dunlocker,Safe Crash Fix,Pirate Perfection Reborn Trainer!Pirate Perfection Reborn Trainer! Free Edition,Pirate Perfection Reborn Trainer! V.I.P. Edition,XP Boost")
		...
		for _, v in pairs(flaggedmods) do
			if string.find(modname, v) then
				managers.chat:_receive_message(1, "Better Anticheat", (tostring(peer:name()))..managers.localization:text("is_using_mod")..modname,tweak_data.system_chat_color)
				log ("[BA]  Mod marcado como trampa encontrado en el jugador "..(tostring(peer:name())))
...
function menu_dump()
	...
	if file then
		...
		file:close()
		log ("[BA]  ConfiguraciÃ³n guardada")
	end
...
if PDA.settings.vactest == true and PDA.settings.pda_pause == false then
	log ("[BA]  VAC test comenzado para el jugador: "..(tostring(peer:name())))
	...
	dohttpreq("http://steamcommunity.com/profiles/"..user_id.."/?xml=1",
		function (page)
			...
			if string.find(page,"vacBanned") ~= nil then
				...
				if vacBanned > 0 then	
					log ("VAC ban encontrado")
					dohttpreq("http://steamcommunity.com/profiles/"..user_id.."/?l=english",
					function (in_page)
						in_page = tostring(in_page)
						if not in_page:find('VAC ban grabÃ¡ndose') then
							return
						end
						local vacBannedDays = tostring(string.match(in_page, '(%d+) dÃ­as%(s%) desde el Ãºltimo ban'))
				...
				log ("[BA]  NO VAC en el jugador en los Ãºltimos 3 meses: "..(tostring(peer:name())))
				end	
			end
		end)
	end
...
log ("[BA] Perfil de Steam siendo comprobado")
	if peer then
		...
		log ("[BA]  No es un jugador de infamia legal el usuario: "..(tostring(peer:name())))
		vactest(peer)
		end
	end
end
...
if PDA.settings.modtest == true and PDA.settings.pda_pause == false then
log ("[BA] Test de MODs especializado comenzado para el jugador: "..(tostring(peer:name())))
	for i, mod in ipairs(peer:synced_mods()) do
		...
		for _, v in pairs(specialmods) do
			if string.find(smodname, v) then
				log ("[BA] Mod trampa encontrado en el jugador: "..(tostring(peer:name())))
				managers.chat:_receive_message(1, "Better Anticheat", (tostring(peer:name()))..managers.localization:text("is_using_mod")..smodname,tweak_data.system_chat_color)
				punishment (peer,"mods")
			end
		end
	end
